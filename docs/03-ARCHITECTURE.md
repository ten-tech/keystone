...
| `ks-broker` | service privilégié | Windows visé ; compile aussi ailleurs, sans effet | élevé | 🔨 verbes énumérés + **sept** barrières SEC-02, 7 tests — aucun verbe implémenté |
| `ks-agent-linux` | agent satellite | Linux musl | aucun | 🔨 scan local, 0 test |

**88 tests au total**, tous portables et tous exécutés — `cargo test --workspace`,
