...
| `ks-collectors` | collecte **lecture seule** | portable ; matériel, inventaire logiciel, posture par le registre **et état effectif par WMI** — TPM, BitLocker et SMART restent hors de portée sans élévation (Phase 2) | aucun | ✅ 48 tests unitaires, 4 collecteurs |
| `ks-cli` | la CLI `ks`, surface de référence | Windows (et Linux pour le dev) | aucun | ✅ `scan`/`status`/`explain`/`journal`/`report`, 23 tests unitaires + **7 d'intégration** (binaire lancé en sous-processus) |
...
**125 tests au total**, tous portables et tous exécutés — `cargo test --workspace`,
