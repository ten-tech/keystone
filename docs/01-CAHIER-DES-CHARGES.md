...
| D5-06 | Surveillance de la posture des protections : Defender (temps réel, exclusions, règles ASR), pare-feu, LSA protection, HVCI, BitLocker, WDAC. Toute désactivation est une alerte immédiate. *S'y ajoute le dernier passage de l'outil de suppression de logiciels malveillants livré par Windows Update (`security.defender.removal_tool.last_run_at`), lu dans son journal. C'est un fait de **maintenance**, jamais un résultat d'analyse : l'outil s'exécute en silence et n'affiche rien quand il ne trouve rien, si bien que son absence prolongée ne produit aucun signal. Keystone ne cherche aucun logiciel malveillant et n'en cherchera pas — l'antivirus et l'analyse de malware figurent nommément au §4.3 parmi ce que ce projet refuse.* |
